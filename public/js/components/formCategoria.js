export default (add = true, data = {})=>{
    ...
    form.addEventListener('submit', e=> {
        ...
        if( add ) {
            ...
            fetch(api(`/api/categoria?${ paramQueries( queries ) }`), { method : 'POST', body : JSON.stringify( data ) })
                .then( res => res.json() )
                .then( res => {
            ...
            fetch( api(`/api/categoria?${ paramQueries( queries ) }`), { method : 'PATCH', body : JSON.stringify( data ) } )
                .then( res => res.json() )
                .then(res => {
                    ElementComponent.dispatchEvent(new CustomEvent('_submit', { detail: { status : res } }))
