...
export default ( params )=>{
        ...
        <div class="div_FJZ0jdm">
            ...
            <header id="header" class="header_N8p7RP6">
                ...
                <div class="div_349Zfgp scroll-h">
                    <a href="#/marca" class="button_0530xdO pointer">${ Icon.get('fi fi-rr-angle-left') }</a>
                    <h4 class="text-ellipsis">Informacion de marca</h4>
                </div>
    ...
    elements.confirm.addEventListener('_click', ()=> {
        ...
        fetch( api(`/api/marca?${ paramQueries( queries ) }`), { method : 'DELETE' } )
            .then( res => res.json() )
            .then(res => {
                if(res) {
                    location.hash = '#/marca'
    ...
    const dataRenderElementItemData =( data = null )=>{
        ...
        if( elements.form == '' ) {
            elements.form = formMarca( false, data )
